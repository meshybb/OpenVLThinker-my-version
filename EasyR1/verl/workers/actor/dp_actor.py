...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto) -> dict[str, Any]:
        ...
        select_keys.extend(["task_idx_mask_tensor"])
        non_tensor_select_keys = ["multi_modal_inputs"]
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    task_idx_mask_tensor_micro = model_inputs["task_idx_mask_tensor"]

                    # all return: (bsz, response_length)
                    log_probs = self._forward_micro_batch(model_inputs, temperature=temperature)

                    pg_loss, pg_metrics, pg_entropy_reg_loss = compute_policy_loss(
                        old_log_probs=old_log_probs,
                        log_probs=log_probs,
                        advantages=advantages,
                        response_mask=response_mask,
                        task_idx_mask_tensor_micro= task_idx_mask_tensor_micro,
                        ...
                        use_entropy_loss=self.config.use_entropy_loss,
                    )
                    ...
                    elif self.config.use_entropy_loss:
                        loss = pg_loss + pg_entropy_reg_loss * 0.02  #0.1 # 0.05 #0.02
                        metrics["actor/entropy_loss_regular"] = pg_entropy_reg_loss.detach().item()
                    else:
                        loss = pg_loss
