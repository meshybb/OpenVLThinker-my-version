...
def compute_data_metrics(batch: DataProto, use_critic: bool = False) -> dict[str, Any]:
    ...
    sequence_score_structure = batch.batch['token_level_scores_structure'].sum(-1)
    ...
    return {
        ...
        # structure score
        'critic/structure_score/mean':
            torch.mean(sequence_score_structure).detach().item(),
        'critic/structure_score/max':
            torch.max(sequence_score_structure).detach().item(),
        'critic/structure_score/min':
            torch.min(sequence_score_structure).detach().item(),
        # length score
