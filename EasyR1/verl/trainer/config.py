...
class DataConfig:
    ...
    def post_init(self):
        ...
        if self.val_image_dir is not None:
            if os.path.exists(self.val_image_dir):  # ray job uses absolute path
                self.val_image_dir = os.path.abspath(self.val_image_dir)
            else:
                print(f"Image directory {self.val_image_dir} not found.")
