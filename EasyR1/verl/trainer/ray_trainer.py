...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    ...
    if "token_level_scores_format" in data.batch:
        adv_inputs["token_level_scores_format"] = data.batch["token_level_scores_format"]
        adv_inputs["token_level_scores_correctness"] = data.batch["token_level_scores_correctness"]
        adv_inputs["token_level_scores_structure"] = data.batch["token_level_scores_structure"]
        adv_inputs["token_level_scores_length"] = data.batch["token_level_scores_length"]
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        val_dataloaders: list[StatefulDataLoader],
    ...
    def _log_train_generations(
        self, inputs: list[str], outputs: list[str], labels: list[str], problem_ids: list[int], scores: list[float]
    ) -> None:

        # Create tuples of (input, output, score) and sort by input text
        samples = list(zip(inputs, outputs, labels, problem_ids, scores))
        # samples.sort(key=lambda x: x[0])  # Sort by input text
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        print("len(self.val_dataloaders)", len(self.val_dataloaders))
        val_reward_metrics = {}
        val_length_metrics = {}
        for task_key, cur_val_dataloader in self.val_dataloaders.items():
            # for batch_dict in self.val_dataloader:
            reward_metrics_lst = defaultdict(list)
            length_metrics_lst = defaultdict(list)
            for batch_dict in cur_val_dataloader:
                test_batch = DataProto.from_single_dict(batch_dict)
                print("len(self.test_batch)", len(test_batch))
                ...
                # reward_tensor, reward_metrics, format_tensor, correctness_tensor, structure_reward_tensor= ray.get(self.val_reward_fn.compute_reward.remote(test_batch))
                reward_tensor, reward_metrics, format_tensor, correctness_tensor, structure_reward_tensor, length_reward_tensor, task_idx_mask_tensor = ray.get(self.val_reward_fn.compute_reward.remote(test_batch))
            ...
            val_reward_metrics_per_task = {f"val/{task_key}_{key}_reward": value for key, value in reduce_metrics(reward_metrics_lst).items()}
            val_length_metrics_per_task = {f"val_{task_key}_{key}": value for key, value in reduce_metrics(length_metrics_lst).items()}
            val_reward_metrics.update(val_reward_metrics_per_task)
            val_length_metrics.update(val_length_metrics_per_task)
        ...
        print(f"Validation time: {str(datetime.timedelta(seconds=time_for_eval))}")
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            if self.config.algorithm.online_filtering:
                ...
                # reward_tensor, reward_metrics, format_tensor, correctness_tensor, structure_reward_tensor= ray.get(self.reward_fn.compute_reward.remote(new_batch))
                reward_tensor, reward_metrics, format_tensor, correctness_tensor, structure_reward_tensor, length_reward_tensor, task_idx_mask_tensor = ray.get(self.reward_fn.compute_reward.remote(new_batch))

                ## new_batch has the task_type 
                ...
                new_batch.batch["token_level_scores_structure"] = structure_reward_tensor
                new_batch.batch["token_level_scores_length"] = length_reward_tensor
                new_batch.batch["task_idx_mask_tensor"] = task_idx_mask_tensor
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        ...
                        # reward_tensor, reward_metrics, format_tensor, correctness_tensor, structure_reward_tensor= ray.get(reward_ref)
                        reward_tensor, reward_metrics, format_tensor, correctness_tensor, structure_reward_tensor, length_reward_tensor, task_idx_mask_tensor= ray.get(reward_ref)

                        batch.batch["token_level_scores"] = reward_tensor
                        batch.batch["token_level_scores_format"] = format_tensor
                        batch.batch["token_level_scores_correctness"] = correctness_tensor
                        batch.batch["token_level_scores_structure"] = structure_reward_tensor
                        batch.batch["token_level_scores_length"] = length_reward_tensor
                        batch.batch["task_idx_mask_tensor"] = task_idx_mask_tensor
