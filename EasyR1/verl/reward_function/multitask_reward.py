...
def _get_task_key(item: Dict[str, Any]) -> str:
    """Helper to generate a consistent key for task types."""
    pt = str(item.get("problem_type", "default"))
    dt = str(item.get("data_type", "")).lower()
    # If segmentation/spatial, distinction by data type (video vs image) matters
    if "segmentation" in pt or "grounding" in pt: 
        return f"{pt}/{dt}" if dt else pt
    return pt
...
THINK_ANSWER_PATTERN = re.compile(
    r"\A\s*<thinking>.*?</thinking>\s*<answer>.*?</answer>\s*\Z",
    re.DOTALL
)
...
def calculate_envelope_score(length: int, envelope: Dict[str, int]) -> float:
    """Computes the trapezoidal reward."""
    a, b, c, d = envelope['min_start'], envelope['peak_start'], envelope['peak_end'], envelope['max_end']
    
    if length < a: return 0.0
    if length > d: return 0.0
    
    # Ramp Up (Overshort Protection)
    if a <= length < b:
        return (length - a) / (b - a)
    ...
    # Ramp Down (Overlong Protection)
    if c < length <= d:
        return 1.0 - (length - c) / (d - c)
...
def compute_score(
    ...
    length_penalty_weight: float = 0, # 0.1 
    ...
    """
    Batch interface.
    Returns: list of dict with keys {overall, format, accuracy, structure_reward, length_reward}
    """
    ...
    for idx, item in enumerate(reward_inputs):
        try:
            ...
            # task_key = _get_task_key(item)
            task_key = item.get("task_type", "") or ""
            ...
            # l_score = calculate_envelope_score(curr_len, envelope)
            l_score = 0

            # Combined Score
            overall = (
                (1.0 - format_weight - length_penalty_weight) * a_score 
                + format_weight * f_score 
                + s_reward
                # + length_penalty_weight * l_score
            )
            ...
            results.append({
                ...
                "length_reward": float(l_score), 
                "task_index": int(task_type_to_id[task_key])
            })
            
        except Exception:
            results.append({
                "overall": 0.0, "format": 0.0, "accuracy": 0.0, 
                "structure_reward": 0.0, "length_reward": 0.0, "task_index": int(task_type_to_id[task_key])
            })

    if random.random() < 0.01:
        for idx, item in enumerate(reward_inputs):
            print(f"Type: {item.get('problem_type', '')} | Len: {len(item.get('response', ''))//4}")
            # print(f"Target Len: {_TASK_LENGTH_MEMORY.get_target_length(_get_task_key(item)):.1f}")
            print({
                "overall": results[idx]["overall"],
                "acc": results[idx]["accuracy"],
                "len_r": results[idx].get("length_reward", 0),
            })
